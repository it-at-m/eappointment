...
class Authority extends Base
{
    ...
    {
        if ($this['locations'] instanceof \BO\Zmsdldb\Collection\Locations) {
    ...
    public static function create($name)
    {
        $data = array(
            'name' => $name,
            'locations' => new \BO\Zmsdldb\Collection\Locations()
        );
    ...
    public function getLocationListByOfficePath($officepath)
    {
        ...
        if (count($authority['locations'])) {
            $locations = new \BO\Zmsdldb\Collection\Locations($authority['locations']);
