...
use BO\Zmsdldb\MySQL\Entity\Service as Entity;
use BO\Zmsdldb\MySQL\Collection\Services as Collection;
use BO\Zmsdldb\Elastic\Service as Base
;
use Error;
    ...
    public function fetchId($service_id)
    {
        try {
            if ($service_id) {
                ...
                $stm->setFetchMode(\PDO::FETCH_CLASS | \PDO::FETCH_PROPS_LATE, '\\BO\\Zmsdldb\\MySQL\\Entity\\Service');
                $stm->execute($sqlArgs);
    ...
    public function fetchList($location_csv = false, $mixLanguages = false)
    {
        ...
        try {
            ...
            $stm->fetchAll(\PDO::FETCH_FUNC, function ($data_json) use ($serviceList) {
                $service = new \BO\Zmsdldb\MySQL\Entity\Service();
