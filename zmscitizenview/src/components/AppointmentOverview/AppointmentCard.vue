<template>
  ...
  >
      ...
      >
        <div class="m-teaser-vertical__date-range">
          <time class="m-teaser-vertical__date-range__item">
            <span class="m-teaser-vertical__date-range__item__day">{{ formatDay() }}</span>
            <span class="m-teaser-vertical__date-range__item__month">{{ formatMonth() }}</span>
          </time>
        </div>
      </div>
    </template>
    <template #content>
      ...
      <b>{{ t('appointmentOverview.appointmentNumber') }}:</b> {{ appointment.processId }}
    </template>
  </muc-card>
</template>
...
const formatTitle = (): string => {
  const serviceTitle = props.appointment.serviceCount + "x " + props.appointment.serviceName;
  const subserviceTitle = props.appointment.subRequestCounts.map(subCount =>
    subCount.count + "x " + subCount.name
  ).join("\n")
  return serviceTitle + "\n" + subserviceTitle;
}
