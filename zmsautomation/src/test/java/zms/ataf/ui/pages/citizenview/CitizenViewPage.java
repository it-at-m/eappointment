    ...
    private boolean deepAddSubserviceOnceByName(String subserviceLabel) {
        ...
        String script =
                "var label='" + esc + "';"
                        ...
                        + "  function ascendToCounter(el){"
                        + "    while(el){"
                        + "      if(el.id&&el.id.indexOf('service-')===0)return el;"
                        + "      var tag=(el.tagName||'').toUpperCase();"
                        + "      if(tag==='MUC-COUNTER')return el;"
                        + "      var root=el.getRootNode&&el.getRootNode();"
                        + "      if(root&&root.host){el=root.host;}else{el=el.parentNode;}"
                        + "    }"
                        + "    return null;"
                        ...
                        + "      var el=nodes[i];"
                        + "      var txt=key(el.textContent||'');"
                        + "      if(txt.indexOf(labelKey)>=0){"
                        + "        var c=ascendToCounter(el);"
                        + "        if(c)return c;"
                        + "      }"
                        + "      if(el.shadowRoot){"
                        + "        var stxt=key(el.shadowRoot.textContent||'');"
                        + "        if(stxt.indexOf(labelKey)>=0){"
                        + "          var c2=ascendToCounter(el);"
                        + "          if(c2)return c2;"
                        + "        }"
                        + "      }"
                        + "    }"
