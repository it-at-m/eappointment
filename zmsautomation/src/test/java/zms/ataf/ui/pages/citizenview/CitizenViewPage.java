...
public class CitizenViewPage extends BasePage {
    ...
    /** Clicks {@code window.__zmsCitizenViewSlotTarget} with synthetic mouse events; falls back to stored slot id. */
    private static final String CLICK_STORED_TIMESLOT_SCRIPT =
            "var t=window.__zmsCitizenViewSlotTarget;"
                    + "var sid=window.__zmsCitizenViewSlotId||'';"
                    ...
                    + " if(node.shadowRoot){var b=node.shadowRoot.querySelector('button:not([disabled])');if(fireClick(b))return true;}}"
                    + " return fireClick(node);"
                    + "}"
                    + "function findByIdDeep(id){"
                    + " function find(root){if(!root)return null;"
                    + " try{var q=root.querySelector('#'+id.replace(/:/g,'\\\\:'));if(q)return q;}catch(e){}"
                    + " var all=root.querySelectorAll('*');"
                    + " for(var i=0;i<all.length;i++){if(all[i].shadowRoot){var f=find(all[i].shadowRoot);if(f)return f;}}"
                    + " return null;}"
                    + " return find(document.body)||document.getElementById(id);"
                    + "}"
                    + "var ok=false;"
                    + "if(t)ok=clickSlotNode(t);"
                    + "if(!ok&&sid){var el=findByIdDeep(sid);if(el)ok=clickSlotNode(el);}"
