...
Feature: CitizenView: Ruppertstraße shared booking (Ort 10489, Slots 10489 + 10313237)
  As a citizen
  ...
  # data-provider-id / provider-{officeId}-timeslot-* (may be 10489 or 10313237).
  ...
  # --- Ort collapse: jump-in on Haupt shows one Ort (10489); Ausbildung 10313237 is not a separate checkbox;
  #     after selecting the Ort, timeslots from both real providers appear in the DOM. ---
  @jumpin @sharedBooking
  Scenario: Wohnsitzanmeldung jump-in 10489 collapses peers and shows slots for 10489 and 10313237
    ...
    And provider checkbox 10313237 should not appear in the citizen view
    When I select office 10489 in the citizen view
    And I wait for appointment slots to be ready in the citizen view
    Then timeslots for providers "10489,10313237" should be visible in the citizen view

  # --- Jump-in on Ausbildung also collapses to display Ort 10489 (shared booking peers). ---
  @jumpin @sharedBooking
  Scenario: Wohnsitzanmeldung jump-in 10313237 still shows display Ort 10489 only
    Given I open zmscitizenview with jump-in service "1063475" and location "10313237"
    Then the service combination step should be visible
    When I continue from the service combination step
    Then provider checkbox 10489 should be visible in the citizen view
    And provider checkbox 10313237 should not appear in the citizen view

  # --- Book a timeslot owned by Ausbildung (10313237) under display Ort 10489; summaries stay on 10313237. ---
  @jumpin @sharedBooking @ausbildungCalendar
  Scenario: Shared booking books Ausbildung timeslot 10313237 under Ort 10489
    Given I open zmscitizenview with jump-in service "1063475" and location "10489"
    Then the service combination step should be visible
    When I continue from the service combination step
    Then provider checkbox 10489 should be visible in the citizen view
    ...
    And the booking summary should show provider 10313237 in the citizen view
  ...
  # --- Haupt-only service (Beglaubigung 1063426): FE must not expand peer 10313237 into the calendar;
  #     timeslots stay on 10489 only. ---
  @jumpin @sharedBooking @mainCalendar
  Scenario: Beglaubigung jump-in 10489 shows timeslots for 10489 only, not Ausbildung 10313237
    ...
    And timeslots for providers "10313237" should not appear in the citizen view

  # --- Haushaltsbescheinigung 1080843 is also at Ausbildung 10313237; combined Personendaten 10224136 is not.
  #     FE must not expand peer 10313237 (would trigger invalidLocationAndServiceCombination). ---
  @jumpin @sharedBooking @mainCalendar
  Scenario: Haushaltsbescheinigung combined with Personendaten shows timeslots for 10489 only, not Ausbildung 10313237
