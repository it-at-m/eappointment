...
CI führt dieselben Suiten mit ParaTest aus, acht Prozesse gleichzeitig (`./vendor/bin/paratest --processes=8`). `zmsbackend` nutzt weiter die eine importierte Datenbank `zmsbo`. `./vendor/bin/phpunit` bleibt der sequenzielle Befehl, auch für `--filter` und `./zmsbackend/zmsbackend-test`.
