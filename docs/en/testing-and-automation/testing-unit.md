...
CI runs the same suites with ParaTest, thirty-two processes at a time (`./vendor/bin/paratest --processes=32`). `./zmsbackend/zmsbackend-test` imports fixtures and then runs one process. Pass `--processes` to raise the count. `./vendor/bin/phpunit` stays the sequential command, including `--filter`.
