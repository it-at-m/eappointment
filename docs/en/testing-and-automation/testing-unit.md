...
CI runs the same suites with ParaTest, eight processes at a time (`./vendor/bin/paratest --processes=8`). `zmsbackend` still uses the one imported `zmsbo` database. `./vendor/bin/phpunit` stays the sequential command, including `--filter` and `./zmsbackend/zmsbackend-test`.
