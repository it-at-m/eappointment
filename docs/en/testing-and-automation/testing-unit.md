...
CI runs the same suites with ParaTest, sixty-four processes at a time (`./vendor/bin/paratest --processes=64`). `zmsbackend` uses four processes on the shared `zmsbo` database. `./zmsbackend/zmsbackend-test` imports fixtures and then runs one process. Pass `--processes` to raise the count. `./vendor/bin/phpunit` stays the sequential command, including `--filter`.
...
PHPUnit 13 prints `N` in the progress line for a PHPUnit notice. In this repository that is usually a mock created with no expectations. `N` does not fail the run. CI does not pass `--fail-on-phpunit-notice`. Use `--display-phpunit-notices` to print the notice text. `--display-notices` is only for PHP notices raised by the test.
...
./zmsbackend/zmsbackend-test                    # Run all tests, 1 process
./zmsbackend/zmsbackend-test --processes=4      # Choose the process count
