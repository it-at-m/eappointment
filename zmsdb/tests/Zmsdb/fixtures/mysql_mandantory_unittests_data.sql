...
UPDATE `citizen` SET `bestaetigt` = 1 WHERE `BuergerID` IN (10118, 10114, 10030);

/* ------------------------------------------------------------------
   Test‑Daten OverallCalendar
-------------------------------------------------------------------*/
LOCK TABLES
  	`overall_calendar` WRITE,
  	`availability`  WRITE;


/* --- Scope 1300 ---------------------------------------------------*/
DELETE FROM `overall_calendar` WHERE scope_id = 101;
INSERT INTO `overall_calendar` (scope_id, time,`availability_id`, seat, status) VALUES
  (101, '2016-05-27 09:30:00', 1550, 1, 'free'),
  (101, '2016-05-27 09:35:00', 1550, 1, 'free');

/* --- Scope 1301 – Availability‑Test ------------------------------*/
DELETE FROM `overall_calendar` WHERE scope_id = 1301;
DELETE FROM `availability`   WHERE OeffnungszeitID = 999;  /* optional */
...
DELETE FROM `overall_calendar` WHERE scope_id = 2001;

INSERT INTO `overall_calendar` (`scope_id`, `availability_id`, `time`, `seat`, `status`, `process_id`, `slots`)
VALUES
