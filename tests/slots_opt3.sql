EXPLAIN
-- SELECT SUM(used), SUM(slots_intern), SUM(slots_public), SUM(slots_callcenter), SUM(free_public), SUM(free_callcenter), SUM(free_intern), Datum, daylabel FROM (
SELECT
...
FROM
    ...
    (SELECT StandortID FROM standort WHERE StandortID IN (321,361,362,363,370,369,368,373,375,381,380,357,352,353,320,312,313,316,333,335,336,337,347,407,384,647,463,466,470,471,468,469,589,593,609,648,451,426,387,386,390,392,404,425,393,400,458,441,610,277,102,136,140,141,142,143,144,145,146,147,148,135,134,101,106,103,107,108,112,109,111,326,133,149,150,151,190,192,231,635,229,289,287,288,303,275,191,172,456,406,153,162,167,168,472,169,170,189,276)) s
    LEFT JOIN oeffnungszeit o USE INDEX (oeff_buerger) ON o.StandortID = s.StandortID
