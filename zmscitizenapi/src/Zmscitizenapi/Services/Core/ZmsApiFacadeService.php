...
class ZmsApiFacadeService
{
    ...
    public static function getOfficeListByServiceId(int $serviceId): OfficeList|array
    {
        ...
        foreach ($requestRelationList as $relation) {
            if ((int) $relation->request->id === $serviceId) {
                ...
                $offices[] = new Office(id: (int) $provider->id, name: $provider->name, address: $provider->address ?? null, geo: $provider->geo ?? null, scope: $scope);
            }
        }
    ...
    public static function getBookableFreeDays(int $officeId, int $serviceId, array $serviceCounts, string $startDate, string $endDate): AvailableDays|array
    {
        ...
        $freeDays = ZmsApiClientService::getFreeDays(new ProviderList([['id' => $officeId, 'source' => \App::$source_name]]), new RequestList([
                [
                    'id' => $serviceId,
                    'source' => \App::$source_name,
                    'slotCount' => $serviceCounts,
                ]
            ]), $firstDay, $lastDay,) ?? new Calendar();
    ...
    public static function getAvailableAppointments(?string $date, ?int $officeId, ?array $serviceIds, ?array $serviceCounts): AvailableAppointments|array
    {
        ...
        $freeSlots = ZmsApiClientService::getFreeTimeslots(new ProviderList([['id' => $officeId, 'source' => \App::$source_name]]), new RequestList($requests), DateTimeFormatHelper::getInternalDateFromISO($date), DateTimeFormatHelper::getInternalDateFromISO($date)) ?? new ProcessList();
