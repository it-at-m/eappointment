...
namespace BO\Zmsbackend\Organisation\Service;
...
class Organisation extends \BO\Zmsbackend\Base
{
    ...
    public function readResolvedReferences(\BO\Zmsentities\Schema\Entity $entity, $resolveReferences)
    {
        if (0 < $resolveReferences && $entity->hasId()) {
            ...
            $entity['ticketprinters'] = (new \BO\Zmsbackend\Ticketprinter\Service\Ticketprinter())
                ->readByOrganisationId($entity->id, $resolveReferences - 1);
        }
        return $entity;
    }
    ...
    public function readByClusterId($clusterId, $resolveReferences = 0)
    {
        ...
        if (! $scope) {
            throw new \BO\Zmsbackend\Cluster\Exception\ClusterWithoutScopes();
        }
    ...
    public function readByHash($hash)
    {
        $organisationId = $this->getReader()
            ->fetchValue((new \BO\Zmsbackend\Ticketprinter\Repository\Ticketprinter(\BO\Zmsbackend\Query\Base::SELECT))
    ...
    public function deleteEntity($itemId)
    {
        ...
        if (0 < $entity->toProperty()->departments->get()->count()) {
            throw new \BO\Zmsbackend\Organisation\Exception\DepartmentListNotEmpty();
        }
        $query = new \BO\Zmsbackend\Organisation\Repository\Organisation(\BO\Zmsbackend\Query\Base::DELETE);
    ...
    {
        $query = new \BO\Zmsbackend\Organisation\Repository\Organisation(\BO\Zmsbackend\Query\Base::INSERT);
    ...
    {
        $query = new \BO\Zmsbackend\Organisation\Repository\Organisation(\BO\Zmsbackend\Query\Base::UPDATE);
