 ...
 */
class ProcessStatusArchived extends \BO\Zmsbackend\Query\Base implements \BO\Zmsbackend\Query\MappingInterface
{
    /**
     *
    ...
    public function addJoinStatisticFailed($dateTime, \BO\Zmsentities\Scope $scope)
    {
        ...
        $this->leftJoin(
            new \BO\Zmsbackend\Query\Alias(self::STATISTIC_TABLE, 'statistic'),
            self::expression('
        ...
        $this->query->where(function (\BO\Zmsbackend\Query\Builder\ConditionBuilder $query) use ($dateTime, $scope) {
            $query->andWith(
