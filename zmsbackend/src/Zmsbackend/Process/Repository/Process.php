...
use BO\Zmsbackend\Scope\Service\Scope as ScopeEntity;
 ...
 */
class Process extends \BO\Zmsbackend\Query\Base implements \BO\Zmsbackend\Query\MappingInterface
{
    /**
    ...
    protected function addJoinAvailability()
    {
        $this->leftJoin(
            ...
            \BO\Zmsbackend\Availability\Repository\Availability::getJoinExpression('`process`', '`availability`')
        );
        $joinQuery = new \BO\Zmsbackend\Availability\Repository\Availability($this, $this->getPrefixed('appointments__0__availability__'));
        return $joinQuery;
    }
    ...
    protected function addJoinScope()
    {
        ...
        $joinQuery = new \BO\Zmsbackend\Query\Scope($this, $this->getPrefixed('scope__'));
        return $joinQuery;
    }
    ...
    public function addConditionScopeIds($scopeIds)
    {
        ...
        $this->query->where(function (\BO\Zmsbackend\Query\Builder\ConditionBuilder $query) use ($scopeIds) {
            $query
    ...
    public function addConditionUpcomingOnly(\DateTimeInterface $now)
    {
        ...
        $this->query->where(function (\BO\Zmsbackend\Query\Builder\ConditionBuilder $condition) use ($today, $time) {
            $condition
                ->andWith('process.Datum', '>', $today)
                ->orWith(function (\BO\Zmsbackend\Query\Builder\ConditionBuilder $inner) use ($today, $time) {
                    $inner
                        ->andWith('process.Datum', '=', $today)
    ...
    public function addConditionScopeNameSearch(string $scopeName)
    {
        ...
        $this->query->where(function (\BO\Zmsbackend\Query\Builder\ConditionBuilder $query) use ($likeValue) {
    ...
    public function postProcess($data)
    {
        ...
        $data[$this->getPrefixed("lastChange")] =
            (new \DateTimeImmutable($data[$this->getPrefixed("lastChange")] .
                \BO\Zmsbackend\Connection\Select::$connectionTimezone))->getTimestamp();
        return $data;
    }
    ...
    protected function addRequiredJoins()
    {
        if ($this->shouldLoadEntity('processuser')) {
            $this->leftJoin(
                new \BO\Zmsbackend\Query\Alias(\BO\Zmsbackend\Useraccount\Repository\Useraccount::TABLE, 'processuser'),
                'process.NutzerID',
                '=',
        ...
        if ($this->shouldLoadEntity('processscope')) {
            $this->leftJoin(
                new \BO\Zmsbackend\Query\Alias(\BO\Zmsbackend\Query\Scope::TABLE, 'processscope'),
                'process.StandortID',
                '=',
