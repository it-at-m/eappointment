...
use BO\Zmsbackend\Helper\ProcessStatus;
...
class Process extends \BO\Zmsbackend\Base implements \BO\Zmsbackend\Interfaces\ResolveReferences
{
    public function readEntity($processId = null, $authKey = null, $resolveReferences = 2)
    {
        ...
        if (!$authKey instanceof \BO\Zmsbackend\Helper\NoAuth) {
    ...
    public function readResolvedReferences(\BO\Zmsentities\Schema\Entity $entity, $resolveReferences)
    {
        if (0 <= $resolveReferences) {
            if ($entity->archiveId) {
                $entity->requests = (new \BO\Zmsbackend\Request\Service\Request())
    ...
    public function updateEntity(\BO\Zmsentities\Process $process, \DateTimeInterface $now, $resolveReferences = 0, $previousStatus = null, ?\BO\Zmsentities\Useraccount $useraccount = null)
    {
        ...
        if (!$process->getId()) {
            throw new \BO\Zmsbackend\Process\Exception\ProcessUpdateFailed();
        }
        $this->perform(\BO\Zmsbackend\Process\Repository\Process::QUERY_UPDATE_FOLLOWING_PROCESS, [
            'reserved' => ($process->status == 'reserved') ? 1 : 0,
            'processID' => $process->getId(),
        ]);
        \BO\Zmsbackend\Log\Service\Log::writeProcessLog("UPDATE (\BO\Zmsbackend\Process\Service\Process::updateEntity) $process ", \BO\Zmsbackend\Log\Service\Log::ACTION_EDITED, $process, $useraccount);
        return $process;
    }
    ...
    public function updateEntityDisplayNumber(Entity $process)
    {
        ...
        \BO\Zmsbackend\Log\Service\Log::writeProcessLog("UPDATE (updateEntityDisplayNumber) $process ", \BO\Zmsbackend\Log\Service\Log::ACTION_EDITED, $process);
        return $process;
    }
    ...
    public function updateEntityWithSlots(\BO\Zmsentities\Process $process, \DateTimeInterface $now, $slotType = "intern", $slotsRequired = 0, $resolveReferences = 0, $userAccount = null)
    {
        ...
        $slotList = (new \BO\Zmsbackend\Slot\Service\Slot())->readByAppointment($appointment, $slotsRequired, true);
        ...
        \BO\Zmsbackend\Log\Service\Log::writeProcessLog("CREATE (\BO\Zmsbackend\Process\Service\Process::updateEntityWithSlots) $process ", \BO\Zmsbackend\Log\Service\Log::ACTION_EDITED, $process, $userAccount);
...
//reserve new appointment
        $processNew = \BO\Zmsbackend\Process\Service\ProcessStatusFree::init()
...
//delete slot mapping for temp process id
        (new \BO\Zmsbackend\Slot\Service\Slot())->deleteSlotProcessMappingFor($processTempNewId);
//write new slot mapping for changed process with old credentials because new appointment data
        (new \BO\Zmsbackend\Slot\Service\Slot())->writeSlotProcessMappingFor($processNew->getId());
        \BO\Zmsbackend\Log\Service\Log::writeProcessLog("UPDATE (\BO\Zmsbackend\Process\Service\Process::writeEntityWithNewAppointment) $process ", \BO\Zmsbackend\Log\Service\Log::ACTION_EDITED, $process);
        ...
        if ($processEntityList->count()) {
            foreach ($processEntityList as $entity) {
                if ($entity->getId() != $processId) {
                    $this->perform(\BO\Zmsbackend\Process\Repository\Process::QUERY_REASSIGN_FOLLWING_PROCESS, [
                        'newProcessId' => $processData->getId(),
                        'processId' => $processId
                    ]);
                }
    ...
    protected function writeNewProcess(\BO\Zmsentities\Process $process, \DateTimeInterface $dateTime, $parentProcess = 0, $childProcessCount = 0, $retry = true, $userAccount = null)
    {
        ...
        try {
            $this->writeItem($query);
        } catch (\BO\Zmsbackend\Exception\Pdo\PDOFailed $exception) {
            if ($retry) {
        // First try might fail if two processes are created with the same number at the same time
                sleep(1);
        // Let the other process complete his transaction
                return $this->writeNewProcess($process, $dateTime, $parentProcess, $childProcessCount, false, $userAccount);
            }
            throw new \BO\Zmsbackend\Process\Exception\ProcessCreateFailed($exception->getMessage());
        }
        (new \BO\Zmsbackend\Slot\Service\Slot())->writeSlotProcessMappingFor($process->id);
        $checksum = ($userAccount) ? sha1($process->id . '-' . $userAccount->getId()) : '';

        \BO\Zmsbackend\Log\Service\Log::writeProcessLog("CREATE (\BO\Zmsbackend\Process\Service\Process::writeNewProcess) $process $checksum ", \BO\Zmsbackend\Log\Service\Log::ACTION_NEW, $process, $userAccount);
        return $process;
    }
    ...
    /**
     * Build a process list without per-row request queries (for batch attach).
     */
    ...
    /**
     * Attach requests for all processes in one query (avoids N+1 in list endpoints).
     */
    ...
    {
        $cluster = (new \BO\Zmsbackend\Cluster\Service\Cluster())->readEntity($clusterId, 1);
        $scopeIds = [];
     ...
     * Regulär sollte aber \BO\Zmsbackend\Process\Service\ProcessStatusArchived::writeEntityFinished()
    ...
    public function writeDeletedEntity($processId): bool
    {
        ...
        if ($processEntityList->count()) {
            foreach ($processEntityList as $entity) {
                ...
                $query = \BO\Zmsbackend\Process\Repository\Process::QUERY_DELETE;
                ...
                if ($status) {
                            ...
                            (new \BO\Zmsbackend\Slot\Service\Slot())->deleteSlotProcessMappingFor($entityId);
                            \BO\Zmsbackend\Log\Service\Log::writeProcessLog("DELETE (\BO\Zmsbackend\Process\Service\Process::writeDeletedEntity) $entityId ", \BO\Zmsbackend\Log\Service\Log::ACTION_DELETED, $entity);
                }
            }
        }
    ...
    public function writeCanceledEntity($processId, $authKey, $now = null, ?\BO\Zmsentities\Useraccount $useraccount = null)
    {
        ...
        $query = \BO\Zmsbackend\Process\Repository\Process::QUERY_CANCELED;
        $this->perform($query, [
            'processId' => $processId,
            'authKey' => $authKey,
        ...
        $process = $this->readEntity($processId, new \BO\Zmsbackend\Helper\NoAuth(), 0);
        \BO\Zmsbackend\Log\Service\Log::writeProcessLog("DELETE (\BO\Zmsbackend\Process\Service\Process::writeCanceledEntity) $processId ", \BO\Zmsbackend\Log\Service\Log::ACTION_CANCELED, $process, $useraccount);
        return $process;
    }
    ...
    public function writeBlockedEntity(\BO\Zmsentities\Process $process, bool $releaseSlots = false, ?\BO\Zmsentities\Useraccount $useraccount = null)
    {
        ...
        $query = \BO\Zmsbackend\Process\Repository\Process::QUERY_DEREFERENCED;
        $status = $this->perform($query, array(
            $amendment,
            $customTextfield,
            $customTextfield2,
        ...
        if ($status) {
            ...
            if ($processEntityList->count()) {
                foreach ($processEntityList as $entity) {
                    ...
                    \BO\Zmsbackend\Log\Service\Log::writeProcessLog("DELETE (\BO\Zmsbackend\Process\Service\Process::writeBlockedEntity) $entityId ", \BO\Zmsbackend\Log\Service\Log::ACTION_DELETED, $process, $useraccount);
                }
            }
        }
        return $status;
    ...
    protected function writeRequestsToDb(\BO\Zmsentities\Process $process)
    {
        ...
        if ($hasRequests) {
            $query = new \BO\Zmsbackend\Request\Repository\XRequest(\BO\Zmsbackend\Query\Base::INSERT);
            foreach ($process->requests as $request) {
                if ($request->id >= 0) {
        // allow deleting requests with a -1 request
                    $query->addValues([
    ...
    protected function deleteRequestsForProcessId($processId)
    {
        $status = false;
        if (0 < $processId) {
            $query = new \BO\Zmsbackend\Request\Repository\XRequest(\BO\Zmsbackend\Query\Base::DELETE);
    ...
    public function isAppointmentAllowedWithSameMail(Entity $entity): bool
    {
        ...
        $processes = $this->readProcessListByMailAddress($entity->getClients()->getFirst()->email, $entity->scope->id);
