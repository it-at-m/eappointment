...
class ProcessStatusArchived extends Process
{
    ...
    public function writeNewArchivedProcess(
        ...
        $query = new \BO\Zmsbackend\Process\Repository\ProcessStatusArchived(\BO\Zmsbackend\Query\Base::INSERT);
        $query->addValuesNewArchive($process, $now);
        $this->writeItem($query);
        ...
        \BO\Zmsbackend\Log\Service\Log::writeProcessLog(
            ...
            \BO\Zmsbackend\Log\Service\Log::ACTION_ARCHIVED,
            $process
        );

        if ($calculateStatistic) {
            (new \BO\Zmsbackend\Exchange\Service\ExchangeWaitingscope())->updateWaitingStatistics($process, $now);
        }
    ...
    {
        $query = new \BO\Zmsbackend\Process\Repository\ProcessStatusArchived(\BO\Zmsbackend\Query\Base::UPDATE);
