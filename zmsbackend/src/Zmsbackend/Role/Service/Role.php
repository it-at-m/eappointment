...
namespace BO\Zmsbackend\Role\Service;
...
class Role extends \BO\Zmsbackend\Base
{
    ...
    public function addRole(Entity $role, int $resolveReferences = 1): ?Entity
    {
        ...
        $query = new \BO\Zmsbackend\Role\Repository\Role(\BO\Zmsbackend\Query\Base::INSERT);
        $query->addValues(
            [
                'name' => $role['name'],
    ...
    public function updateRole(int $roleId, Entity $role, int $resolveReferences = 1): ?Entity
    {
        ...
        $query = new \BO\Zmsbackend\Role\Repository\Role(\BO\Zmsbackend\Query\Base::UPDATE);
        ...
        $delLinks = new \BO\Zmsbackend\Role\Repository\RolePermission(\BO\Zmsbackend\Query\Base::DELETE);
    ...
    private function fetchPermissionsByNamesOrFail(array $permissionNames): array
    {
        ...
        $permQuery = new \BO\Zmsbackend\Permission\Repository\Permission(\BO\Zmsbackend\Query\Base::SELECT);
