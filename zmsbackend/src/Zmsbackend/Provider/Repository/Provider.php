...
namespace BO\Zmsbackend\Provider\Repository;
...
class Provider extends \BO\Zmsbackend\Query\Base
{
    ...
    public function addConditionRequestCsv($requestIdCsv, $sourceName = 'dldb')
    {
        ...
        $this->leftJoin(
            new \BO\Zmsbackend\Query\Alias("request_provider", 'xprovider'),
            'provider.id',
            '=',
            'xprovider.provider__id'
        );
        $this->query->where(function (\BO\Zmsbackend\Query\Builder\ConditionBuilder $query) use ($requestIdList, $sourceName) {
