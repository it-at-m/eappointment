...
namespace BO\Zmsbackend\Provider\Service;
...
class Provider extends \BO\Zmsbackend\Base
{
    ...
    /** @psalm-api */
    public function readEntityById($providerId, int $resolveReferences = 0, bool $disableCache = false)
    {
    ...
    public function writeEntity(Entity $entity)
    {
        ...
        if (! $contact->count()) {
            throw new \BO\Zmsbackend\Provider\Exception\ProviderContactMissed();
        }
        $query = new \BO\Zmsbackend\Provider\Repository\Provider(\BO\Zmsbackend\Query\Base::INSERT);
        ...
        $query->addValues([
            ...
            'contact__postalCode' => (string) $contact->getProperty('postalCode'),
    ...
    public function writeImportEntity($provider, $source = 'dldb')
    {
        if ($provider['address']['postal_code']) {
            $query = new \BO\Zmsbackend\Provider\Repository\Provider(\BO\Zmsbackend\Query\Base::REPLACE);
            $query->addValues([
                'source' => $source,
                'id' => $provider['id'],
                'name' => $provider['name'],
                ...
                'contact__postalCode' => (string) $provider['address']['postal_code'],
