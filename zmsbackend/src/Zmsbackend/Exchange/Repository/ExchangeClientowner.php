...
class ExchangeClientowner extends \BO\Zmsbackend\Query\Base
{
    /**
    ...
    FROM (
          SELECT
            ...
            0 as requestcount                
            FROM ' . \BO\Zmsbackend\Owner\Repository\Owner::TABLE . ' o
                LEFT JOIN ' . \BO\Zmsbackend\Department\Repository\Department::TABLE . ' d ON d.`KundenID` = o.`KundenID`
                LEFT JOIN ' . \BO\Zmsbackend\Query\Scope::TABLE . ' scope ON scope.`BehoerdenID` = d.`BehoerdenID`
                LEFT JOIN ' . \BO\Zmsbackend\Process\Repository\ProcessStatusArchived::TABLE . ' a ON a.`StandortID` = scope.`StandortID`
      ...
      UNION ALL  
          SELECT
              ...
              COUNT(IF(ba.AnliegenID > 0, ba.AnliegenID, null)) as requestcount
                FROM ' . \BO\Zmsbackend\Owner\Repository\Owner::TABLE . ' o
                    LEFT JOIN ' . \BO\Zmsbackend\Department\Repository\Department::TABLE . ' d ON d.`KundenID` = o.`KundenID`
                    LEFT JOIN ' . \BO\Zmsbackend\Query\Scope::TABLE . ' as scope ON d.`BehoerdenID` = scope.`BehoerdenID`
                    LEFT JOIN ' . \BO\Zmsbackend\Process\Repository\ProcessStatusArchived::TABLE . ' as a ON scope.`StandortID` = a.`StandortID`
    ...
    const QUERY_SUBJECTS = '
      ...
      FROM ' . \BO\Zmsbackend\Owner\Repository\Owner::TABLE . ' AS o
          INNER JOIN
            (
              SELECT
