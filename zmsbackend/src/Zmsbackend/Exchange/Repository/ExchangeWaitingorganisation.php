...
class ExchangeWaitingorganisation extends \BO\Zmsbackend\Query\Base
{
    /**
    ...
    const QUERY_READ_DAY = '
        SELECT
            w.`datum`,
            ' . \BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::WAITING_VALUES . '
        FROM ' . self::TABLE . ' as w
            LEFT JOIN ' . \BO\Zmsbackend\Query\Scope::TABLE . ' AS s ON w.`standortid` = s.`StandortID`
            LEFT JOIN ' . \BO\Zmsbackend\Department\Repository\Department::TABLE . ' AS d ON s.`BehoerdenID` = d.`BehoerdenID`
            LEFT JOIN ' . \BO\Zmsbackend\Organisation\Repository\Organisation::TABLE . ' AS o ON o.`OrganisationsID` = d.`OrganisationsID`
        WHERE
