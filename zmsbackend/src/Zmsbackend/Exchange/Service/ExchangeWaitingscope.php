...
class ExchangeWaitingscope extends \BO\Zmsbackend\Base implements \BO\Zmsbackend\Interfaces\ExchangeSubject
{
    ...
    public function readEntity(
        ...
        $raw = $this
            ->getReader()
            ->fetchAll(
                constant("\BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::QUERY_READ_" . strtoupper($period)),
                [
    ...
    {
        $raw = $this->getReader()->fetchAll(\BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::QUERY_SUBJECTS, []);
        $entity = new Exchange();
    ...
    public function readPeriodList($subjectid, $period = 'day')
    {
        ...
        $montsList = $this->getReader()->fetchAll(
            constant("\BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::QUERY_PERIODLIST_MONTH"),
            [
                'scopeid' => $subjectid,
            ]
        );
    ...
    public function readByDateTime(
        ...
        $sql = \BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::getQuerySelectByDateTime($date, $isWithAppointment);
        ...
        if (!$existingEntry) {
            $this->perform(
                \BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::QUERY_CREATE,
                [
                    'scopeid' => $scope->id,
                    'date' => $date->format('Y-m-d'),
                ]
    ...
    public function writeWaitingTimeCalculated(
        ...
        $queueList = (new \BO\Zmsbackend\Scope\Service\Scope())->readQueueListWithWaitingTime($scope, $now);
        ...
        $this->perform(
            \BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::getQueryUpdateByDateTime($now, $isWithAppointment),
            [
    ...
    public function writeWaitingTime(
        ...
        $this->perform(
            \BO\Zmsbackend\Exchange\Repository\ExchangeWaitingscope::getQueryUpdateByDateTime(
                $process->getArrivalTime($now),
                $process->isWithAppointment()
            ),
            [
