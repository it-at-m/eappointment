...
class ExchangeWaitingorganisation extends \BO\Zmsbackend\Base implements \BO\Zmsbackend\Interfaces\ExchangeSubject
{
    ...
    public function readEntity(
        ...
        foreach ($subjectIdList as $subjectid) {
            $raw = $this
                ->getReader()
                ->fetchAll(
                    constant("\BO\Zmsbackend\Exchange\Repository\ExchangeWaitingorganisation::QUERY_READ_" . strtoupper($period)),
                    [
    ...
    {
        $raw = $this->getReader()->fetchAll(\BO\Zmsbackend\Exchange\Repository\ExchangeWaitingorganisation::QUERY_SUBJECTS, []);
        $entity = new Exchange();
    ...
    public function readPeriodList($subjectid, $period = 'day')
    {
        ...
        $montsList = $this->getReader()->fetchAll(
            constant("\BO\Zmsbackend\Exchange\Repository\ExchangeWaitingorganisation::QUERY_PERIODLIST_MONTH"),
            [
                'organisationid' => $subjectid,
            ]
        );
