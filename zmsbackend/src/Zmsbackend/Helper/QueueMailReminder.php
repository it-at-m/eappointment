...
use BO\Zmsbackend\Mail\Service\Mail as MailRepository;
...
class QueueMailReminder
{
    ...
    public function __construct(\DateTimeInterface $now, $sendReminderBeforeMinutes, $verbose = false)
    {
        ...
        if ($verbose) {
            $this->verbose = true;
            $this->log(
                "\nINFO: Queue email reminder (Limits: " .
        ...
        if ($process->getFirstClient()->hasEmail() && $department->hasMail()) {
            ...
            if ($commit) {
                ...
                \BO\Zmsbackend\Log\Service\Log::writeProcessLog(
                    "Write Reminder (\BO\Zmsbackend\Mail\Service\Mail::writeInQueue) $entity ",
                    \BO\Zmsbackend\Log\Service\Log::ACTION_SEND_REMINDER,
                    $process
                );
