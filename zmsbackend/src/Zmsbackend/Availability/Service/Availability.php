 ...
 */
class Availability extends \BO\Zmsbackend\Base implements \BO\Zmsbackend\Interfaces\ResolveReferences
{
    public static $cache = [];
    ...
    public function readResolvedReferences(
        ...
        if (1 <= $resolveReferences && $entity->hasId()) {
            if (isset($entity->scope['id'])) {
                $entity['scope'] = (new \BO\Zmsbackend\Scope\Service\Scope())->readEntity(
                    $entity->scope['id'],
                    $resolveReferences - 1,
                    !$preferCache
                );
    ...
    public function readList(
        ...
        if (1 <= $resolveReferences) {
            $scope = (new \BO\Zmsbackend\Scope\Service\Scope())->readEntity($scopeId, $resolveReferences - 1);
        }
    ...
    */
    ...
    public function writeEntity(\BO\Zmsentities\Availability $entity, $resolveReferences = 0)
    {
        self::$cache = [];
        $entity->testValid();
        $query = new \BO\Zmsbackend\Availability\Repository\Availability(\BO\Zmsbackend\Query\Base::INSERT);
        $values = $query->reverseEntityMapping($entity);
        $query->addValues($values);
        if (!$this->writeItem($query)) {
            throw new \BO\Zmsbackend\Availability\Exception\AvailabilityCreateFailed();
        }
    ...
    public function updateEntity($entityId, \BO\Zmsentities\Availability $entity, $resolveReferences = 0)
    {
        self::$cache = [];
        $entity->testValid();
        $query = new \BO\Zmsbackend\Availability\Repository\Availability(\BO\Zmsbackend\Query\Base::UPDATE);
    ...
    {
        self::$cache = [];
        $query =  new \BO\Zmsbackend\Availability\Repository\Availability(\BO\Zmsbackend\Query\Base::DELETE);
