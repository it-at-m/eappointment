...
namespace BO\Zmsbackend\Scope\Service;
 ...
 *
 */
class Scope extends \BO\Zmsbackend\Base
{
    public static $cache = [ ];
    ...
    public function readResolvedReferences(
        ...
        if (0 < $resolveReferences) {
            ...
            $entity['closure'] = (new \BO\Zmsbackend\Availability\Service\Closure())->readByScopeId($entity->id, $disableCache);
        }
        return $entity;
    }
        ...
        if (empty($result)) {
            if ($resolveReferences > 0) {
                ...
                $result = $this->getReader()->perform(
                    (new \BO\Zmsbackend\Query\Scope(\BO\Zmsbackend\Query\Base::SELECT))->getQuerySimpleClusterMatch(),
                    [$clusterId]
                );
            }
    ...
    public function readByDepartmentId($departmentId, $resolveReferences = 0, $disableCache = false)
    {
        ...
        if (empty($result)) {
            if ($resolveReferences > 0) {
                ...
                $result = $this->getReader()->perform(
                    (new \BO\Zmsbackend\Query\Scope(\BO\Zmsbackend\Query\Base::SELECT))->getQuerySimpleDepartmentMatch(),
                    [$departmentId]
                );
            }
        }
    ...
    public function readWaitingNumberUpdated($scopeId, $dateTime, $respectContingent = true)
    {
        ...
        $this->perform(
            (new \BO\Zmsbackend\Query\Scope(\BO\Zmsbackend\Query\Base::SELECT))->getQueryLastWaitingNumber(),
            ['scope_id' => $scopeId]
        );
    ...
    {
        $this->perform(
            (new \BO\Zmsbackend\Query\Scope(\BO\Zmsbackend\Query\Base::SELECT))->getQueryLastDisplayNumber(),
            ['scope_id' => $scopeId]
        );
    ...
    public function readQueueList($scopeIds, $dateTime, $resolveReferences = 0, $withEntities = [])
    {
        if ($resolveReferences > 0) {
            ...
            $queueList = (new \BO\Zmsbackend\Queue\Service\Queue())
    ...
    public function updateEntity($scopeId, \BO\Zmsentities\Scope $entity, $resolveReferences = 0)
    {
        ...
        $query = new \BO\Zmsbackend\Query\Scope(\BO\Zmsbackend\Query\Base::UPDATE);
    ...
    {
        $imageName = 's_' . $scopeId . '_bild';
        return $this->perform((new \BO\Zmsbackend\Query\Scope(\BO\Zmsbackend\Query\Base::DELETE))->getQueryDeleteImage(), array(
            'imagename' => "$imageName%"
        ));
    }
    ...
    public function deleteEntity($scopeId)
    {
        ...
        if (0 < $processListCount) {
            throw new \BO\Zmsbackend\Scope\Exception\ScopeHasProcesses();
        }
        self::$cache = [];
        ...
        $query = new \BO\Zmsbackend\Query\Scope(\BO\Zmsbackend\Query\Base::DELETE);
        $query->addConditionScopeId($scopeId);
