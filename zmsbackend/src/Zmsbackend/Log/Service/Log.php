...
namespace BO\Zmsbackend\Log\Service;
...
class Log extends \BO\Zmsbackend\Base
{
    ...
    public static function writeProcessLog(
        ...
        if (!empty($process->getRequestIds())) {
            $requests = (new \BO\Zmsbackend\Request\Service\Request())->readRequestsByIds($process->getRequestIds());
        }
    ...
    public function clearLogsOlderThan(int $olderThan): bool
    {
        try {
            ...
            $query = new \BO\Zmsbackend\Log\Repository\Log(\BO\Zmsbackend\Query\Base::DELETE);
