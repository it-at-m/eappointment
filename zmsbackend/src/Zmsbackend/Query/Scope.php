...
class Scope extends Base implements MappingInterface
{
    ...
    public function addJoin()
    {
        if ($this->shouldLoadEntity('provider')) {
            $this->leftJoin(
                new \BO\Zmsbackend\Query\Alias(\BO\Zmsbackend\Provider\Repository\Provider::getTablename(), 'provider'),
                self::expression('scope.InfoDienstleisterID = provider.id && scope.source = provider.source')
            );
            $providerQuery = new \BO\Zmsbackend\Provider\Repository\Provider($this, $this->getPrefixed('provider__'));
    ...
    protected function addRequiredJoins()
    {
        ...
        if ($this->shouldLoadEntity('scopedepartment')) {
            $this->leftJoin(
                new \BO\Zmsbackend\Query\Alias(\BO\Zmsbackend\Department\Repository\Department::TABLE, 'scopedepartment'),
                'scope.BehoerdenID',
                '=',
        ...
        if ($this->shouldLoadEntity('scopemail') && $this->shouldLoadEntity('scopedepartment')) {
            $this->leftJoin(
                new \BO\Zmsbackend\Query\Alias('email', 'scopemail'),
                'scopedepartment.BehoerdenID',
                '=',
        ...
        if ($this->shouldLoadEntity('scopeprovider')) {
            $this->leftJoin(
                new \BO\Zmsbackend\Query\Alias(\BO\Zmsbackend\Provider\Repository\Provider::getTablename(), 'scopeprovider'),
    ...
    private function convertTimestamps($data)
    {
        ...
        if ($lastChange !== null) {
            $data[$lastChangeKey] = (new \DateTime($lastChange . \BO\Zmsbackend\Connection\Select::$connectionTimezone))
                ->getTimestamp();
        } else {
