...
use \BO\Zmsbackend\Helper\AppointmentDeallocateByCron;
...
class AppointmentDeallocateByCronTest extends \BO\Zmsbackend\Tests\Service\Base
{
    ...
    public function testWithDeallocatedProcess()
    {
        ...
        $processList = (new \BO\Zmsbackend\Process\Service\Process)->readDeallocateProcessList($now, 10, 0);
        $this->assertEquals(
        ...
        $this->assertEquals(0, (new \BO\Zmsbackend\Process\Service\Process)->readDeallocateProcessList($now, 10, 0)->count());
    }
    ...
    public function testWithoutDeallocatedProcess()
    {
        ...
        $helper->startProcessing(false);
        $this->assertEquals(2, $helper->getCount()['deallocated']);
        $this->assertEquals(2, (new \BO\Zmsbackend\Process\Service\Process)->readDeallocateProcessList($now, 10, 0)->count());
    }
