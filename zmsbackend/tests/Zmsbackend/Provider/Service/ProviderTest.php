...
namespace BO\Zmsbackend\Tests\Provider\Service;

use \BO\Zmsbackend\Provider\Service\Provider as Query;

class ProviderTest extends \BO\Zmsbackend\Tests\Service\Base
{
    ...
    {
        $query = new \BO\Zmsbackend\RequestRelation\Service\RequestRelation();
        $repository = (new \BO\Zmsdldb\FileAccess())->loadFromPath(\BO\Zmsbackend\Source\Zmsdldb::$importPath);
        $importInput = $repository->fromLocation()->fetchList();
        $collection = $query->writeImportList($importInput, 'dldb'); //return written entity by true
        $this->assertEquals('dldb', $collection->getFirst()->getSource());
