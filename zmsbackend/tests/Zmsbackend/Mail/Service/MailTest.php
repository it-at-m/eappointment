...
class MailTest extends \BO\Zmsbackend\Tests\Service\Base
{
    ...
    {
        $now = static::$now;
        $scope = (new \BO\Zmsbackend\Scope\Service\Scope)->readEntity(451, 1); // Mobiles Bürgeramt Reinickendorf with Admin Email
    ...
    {
        $this->expectException('BO\Zmsbackend\Mail\Exception\MailWritePartFailed');
        $now = static::$now;
        $query = new Query();
        $input = $this->getTestEntity();
