...
namespace BO\Zmsbackend\Tests\Cluster\Service;
...
class ClusterTest extends \BO\Zmsbackend\Tests\Service\Base
{
    ...
    public function testReadQueueListWithCallTime()
    {
        ...
        $queueList = (new \BO\Zmsbackend\Scope\Service\Scope())->readQueueList([106], $now);
        $this->assertEquals(1461077847, $queueList->getFirst()->lastCallTime);

        $now = new \DateTimeImmutable("2016-05-16 11:55");
        $queueList = (new \BO\Zmsbackend\Scope\Service\Scope())->readQueueList([141], $now);
