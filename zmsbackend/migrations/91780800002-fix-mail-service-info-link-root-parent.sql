...
--
-- München templates used only the immediate parent_id:
--   {% set requestId = requestGroup['request'].parent_id ? ... : requestGroup['request'].id %}
...
SET `value` = REPLACE(
    `value`,
    '{% set requestId = requestGroup[\'request\'].parent_id ? requestGroup[\'request\'].parent_id : requestGroup[\'request\'].id %}',
    '{% set requestId = requestGroup[\'request\'].root_parent_id %}'
)
WHERE `value` LIKE '%requestGroup[\'request\'].parent_id ? requestGroup[\'request\'].parent_id%';
...
WHERE `value` LIKE '%{{ requestGroup[\'request\'].data.id|default(requestGroup[\'request\'].id) }}%';
