...
use BO\Zmscitizenbackend\Appointment\Exception\MoreThanAllowedAppointmentsPerMail;
...
class AppointmentUpdateRepository
{
    ...
    private function persistClientData(
        Pdo $pdo,
        ThinnedProcess $process,
        ?AuthenticatedUser $user,
        int $processId
        ...
        try {
            $locked = $pdo->fetchOne(AppointmentUpdateQueries::QUERY_LOCK_PROCESS, [
                'processId' => $processId,
            ]);
            ...
            $pdo->perform(AppointmentUpdateQueries::QUERY_UPDATE_CLIENT_DATA, [
                'familyName' => $process->familyName ?? '',
                'email' => $process->email ?? '',
                'telephone' => $process->telephone ?? '',
                'customTextfield' => $process->customTextfield ?? '',
                'customTextfield2' => $process->customTextfield2 ?? '',
                'externalUserId' => $this->resolveExternalUserId($locked, $user),
                'processId' => $processId,
            ]);
     ...
     */
    private function resolveExternalUserId(array $locked, ?AuthenticatedUser $user): mixed
    {
        $externalUserId = $locked['external_user_id'] ?? null;
        if ($user === null) {
            return $externalUserId;
        }
        $incoming = $user->getExternalUserId();
        if (
