...
use BO\Zmscitizenbackend\Appointment\Exception\ExternalUserIdMatchFailed;
...
class AppointmentByIdRepository
{
    ...
    public function readAppointmentById(
        int $processId,
        ?string $authKey,
        ...
        try {
            ...
            $this->assertAccess($processRow, $authKey, $user);
     ...
     */
    private function assertAccess(array $processRow, ?string $authKey, ?AuthenticatedUser $user): void
    {
        if ($authKey !== null) {
            $storedAuthKey = (string) ($processRow['auth_key'] ?? '');
            if ($storedAuthKey !== urldecode($authKey)) {
                throw new AuthKeyMatchFailed();
            }
            return;
        }

        if ($user !== null) {
            $processExternalUserId = $processRow['external_user_id'] ?? null;
            ...
            if (
                ...
                throw new ExternalUserIdMatchFailed();
            }
            return;
        }
